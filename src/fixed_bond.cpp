...
namespace fluent
{
	...
	double FixedBond::ytm(double price)
	{
		...
		double x = (coupon_payment + (_face_value - price) / _maturity) / ((_face_value + price) / 2);
		return x;
	}
	...
	std::vector<fluent::Cashflow> FixedBond::get_cashflows()
	{
		...
		for (int i = 1; i < _maturity + 1; i++)
		{
			...
			fluent::Cashflow cashflow(discounted_cashflow, fluent::Date(1, fluent::Month::February, 2024));
